...
The `pyproject.toml` in each example pins `braintrust` to the local checkout via:

```toml
[tool.uv.sources]
braintrust = { path = "../..", editable = true }
```
...
1. Create `py/examples/<name>/` with the layout above.
