...
def _pipeline_metadata(args: Any, kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    return _clean_nones({"agent_names": agent_names})
...
_agent_call_wrapper = _make_task_wrapper(
    ...
    metadata_fn=lambda instance, _a, _k: _clean_nones({"agent_class": instance.__class__.__name__}),
)
    ...
    with contextlib.ExitStack() as stack:
        span = stack.enter_context(
            start_span(
                ...
                metadata=_clean_nones({"toolkit_class": instance.__class__.__name__}),
            )
        )
        try:
            result = await wrapped(*args, **kwargs)
